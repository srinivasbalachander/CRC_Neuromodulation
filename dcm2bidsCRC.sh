...
The following arguments are OPTIONAL:
	...
	--sub: The DICOM folder name of the subject/session that you want to specifically convert. If this is not specified, then this is attempt to convert all subjects within the DICOM directory for whom conversion has not yet been done.

	--ncores: Number of parallel cores to use for conversion (Default: 1)
...
# Make the DICOM 2 BIDS as one massive task script
...
task(){
	...
	rename "s/$i/sub-${SUBID}_ses-${SES}_run-01/" *
	...
	rename "s/epi_epi/epi/" *

	# Rename the runs properly based on suffixed "a" or "b" generated by dcm2niix

	rename "s/run-01/run-02/" *a.*
	rename "s/run-01/run-03/" *b.*
	rename "s/run-01/run-04/" *c.*

	for extn in .nii.gz .json; do
		rename "s/a${extn}/${extn}/" *a.*
	rename "s/b${extn}/${extn}/" *b.*
	rename "s/c${extn}/${extn}/" *c.*;done
